...
\begin{align}
    ...
    %
    =&\; \rho(\bh) \lambdax_{\bh,\bd}(L)\asymp \rho(\bh)\lambda_{\bd}(L),\label{eq:equiv}
\end{align}
...
\]
where $\omega_{d_j-1}$ stands for the area of the ($d_j-1$)-dimensional hypersphere.
...
\begin{lemma}\label{lemma-in2-e}
    Let $\bX^{(1)}, \dots, \bX^{(r)}$ be mutually independent. Then,
    \begin{align*}
    \mathsf{E}[T_{n,2}]=A_n^{(1)}(1+o(1))%+o\lrp{n^{-1}\sum_{j=1}^r h_j^{-d_j}}
    ,\quad A_n^{(1)}=\sum_{j=1}^r \frac{v_{d_j}(L)R(f_{0,-j})}{nh_j^{d_j}},
\end{align*}
...
\begin{lemma}\label{lemma-var-tn3}
    $\bX^{(1)}, \dots, \bX^{(r)}$ be mutually independent. Then,
    \begin{align*}
...
\[
    n\rho(\bh)^{1/2}
    \left(T_n-\mathsf{E}[T_n]\right)
    \xrightarrow{d}
    \mathcal{N}\left(
        0,
        2\lambda_{\bd}(L)^{-4}\prod_{j=1}^{r}\sigma_j^2
    \right),
\]
where $\sigma_j^2$ is defined in Lemma~\ref{lemma-sigma2}. Moreover,
\[
    \mathsf{E}[T_n]
    =
    \frac{v_{\bd}(L)}{n\rho(\bh)}(1+o(1))
    -
    A_n^{(1)}(1+o(1)).
\]
In particular, since $A_n^{(1)}=o\{(n\rho(\bh))^{-1}\}$,
\[
    \mathsf{E}[T_n]
    =
    \frac{v_{\bd}(L)}{n\rho(\bh)}(1+o(1)).
...
\begin{align*}
    \frac{1}{B} \sum_{i=1}^{B} 1_{\{T_n \le T_n^{\sigma_{,i}}\}}.
\end{align*}
...
\textbf{Mixture of iid von Mises--Fisher distributions:} we simulate a latent random vector $\bmu \sim \mathrm{vMF}_d (\bmu_1, \kappa_1)$, and, then, we simulate $\bX^{(1)}, \dots, \bX^{(r)} \mid \bmu \stackrel {iid}\sim  \mathrm{vMF}_d (\bmu, \kappa)$. This produces the density
    \begin{align*}
...
It is worth noting that, for the case with $r=2, d_1 = d_2 = 1, \kappa_1 = 0$ (two random vectors uniformly distributed in the circle), the joint distribution of the angles belongs to the  Johnson--Wehrly family of distribution --- see \cite{Johnson1978,Wehrly1980}. This result is available in Proposition \ref{prop-js-family}.
...
\textbf{Markovian process:} we simulate $\bX^{(1)} \sim \mathrm{vMF}_d (\bmu_1, \kappa_1)$, and, then, we simulate $\bX^{(j)} | (\bX^{(1)} = \bx^{(1)} , \dots, \bX^{(j-1)} = \bx^{(j-1)}) \sim \mathrm{vMF}_d (\bx^{(j-1)}, \kappa)$ for $j = 2, \dots, r$. This produces the density
    \begin{align*}
...
The power results for the Markov model are available in Table \ref{tab-markov}. Except for large $d$, the test demonstrated good performance over all scenarios.
...
Figure \ref{fig:power_curves_mixture} summarizes some key interesting findings from the simulation study. First, it is possible to see the power increasing as function of the dependence parameter $\kappa$, which is reasonable. Furthermore, it is possible to see that increasing the bandwidth has no relevant effect on the power, under both null and alternative hypotheses. Finally, it is interesting to notice the effect of the dimension parameters $d$ and $r$: the first ore had a considerable negative effect on the power, while - except for $c = 0.5$ - large $r$ surprisingly increased the power under alternative scenarios without affecting type I error control.

\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{plots/power_curves_mixture.pdf}
    \caption{Empirical power as a function of the significance level in the model ``Mixture of iid von Mises--Fisher distributions'', for $n=100$. From first to last line: $(r=2,d=1)$, $(r=2,d=10)$, $(r=10,d=1)$. From left to right: $c = 0.5$, $c=1$, $c=2$. the parameter $\kappa \in [0,\infty)$ represents deviation from independence, and $c$ is a tuning parameter related to the degree of smoothness (see the text in this section). The dashed lines represent a 95\% confidence interval for the power under the null hypothesis ($\kappa = 0$). Axes in log-scale.}
    \label{fig:power_curves_mixture}
\end{figure}

Finally, Figure \ref{fig:power_curves_markov} exhibits power curves for the Markovian model. It is possible to visulise, again, a considerable negative influence of the dimension $d$ on the test power.

\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{plots/power_curves_markov.pdf}
    \caption{Same description as Figure \ref{fig:power_curves_mixture}, but for the model ``Markovian process''.}
    \label{fig:power_curves_markov}
\end{figure}
...
\begin{align*}
    H_0: \bZ^{(1)} \independent \bZ^{(2)},
\end{align*}
and the obtained p-values are shown in Figure \ref{fig:pvalue_comet}.
...
\begin{proof}[Proof of Lemma \ref{lemma-in1}]
...
Using Lemma \ref{lemma-sum-variances}, it follows that

\begin{align*}
    \mathsf{Var}(Z_{ni}) &= O \lrb{\mathsf{Var} \lrb{\int_{\Sr} L_{\bh}^2 (\bx,\bX_i)  \,\rd \bx}} + O \lrb{\mathsf{Var} \lrb{\int_{\Sr} L_{\bh}  (\bx,\bX_i) \mathsf{E} (L_{\bh}  (\bx,\bX_i))  \,\rd \bx}}.
\end{align*}

Note that $\mathsf{E} (L_{\bh}  (\bx,\bX_i))$ is a bounded function, which entails that
\begin{align*}
    \int_{\Sr} L_{\bh}  (\bx,\bX_i) \mathsf{E} (L_{\bh}  (\bx,\bX_i))  \,\rd \bx &\le \lrb{\int_{\Sr} L_{\bh}  (\bx,\bX_i)  \,\rd \bx} \mathsf{sup}_{\bx \in \Sr} (\mathsf{E} (L_{\bh}  (\bx,\bX_i))) \\
    &= c_{\bd,L}(\bh)^{-1} \mathsf{sup}_{\bx \in \Sr} (\mathsf{E} (L_{\bh}  (\bx,\bX_i))) \\
    &= O(1).
\end{align*}
Therefore, it also follows that $\mathsf{Var} \lrb{\int_{\Sr} L_{\bh}  (\bx,\bX_i) \mathsf{E} (L_{\bh}  (\bx,\bX_i))  \,\rd \bx} = O(1)$. Now, by a step similar to Equation \ref{eq:equiv}, see that, for every $\by \in \Sr$,
\begin{align*}
    \int_{\Sr} L_{\bh}^2 (\bx,\by)  \,\rd \bx &= c_{\bd,L}(\bh)^{2} \int_{\Sr}L^2 \lrp{\frac{1-\bx_1' \by_1}{h_1^2},\ldots, \frac{1-\bx_r' \by_r}{h_r^2}}\, \,\rd \bx \nonumber \\
    =& c_{\bd,L}(\bh)^{2} \int_{[-1,1]^r}L^2 \lrp{\frac{1-t_1}{h_1^2},\ldots, \frac{1-t_r}{h_r^2}}\times \prod_{j=1}^r \left(1-t_j^2\right)^{d_j/2-1}\om{d_j-1}\,\rd \bt   \nonumber \\
    %
    =&\; \rho(\bh)^{-1} \frac{\lambda_{\bh,\bd}(L^2)}{\lambda_{\bh,\bd}(L)^2} \asymp \rho(\bh)^{-1} \frac{\lambda_{\bd}(L^2)}{\lambda_{\bd}(L)^2}.
\end{align*}

Therefore,
\begin{align*}
    \mathsf{Var} \lrb{\int_{\Sr} L_{\bh}^2 (\bx,\bX_i)  \,\rd \bx} &\le \mathsf{E} \lrb{\lrb{\int_{\Sr} L_{\bh}^2 (\bx,\bX_i)  \,\rd \bx}^2} \\
    &= O(\rho(\bh)^{-2}).
\end{align*}

Now, taking into account that $I_{n,2} = \frac{1}{n^2} \sum_{i=1}^{n} Z_{ni}$, where $Z_{ni}$ are iid terms, it follows that
\begin{align*}
    \mathsf{Var} (I_{n,2}) &= \frac{1}{n^3} \mathsf{Var} (Z_{ni}) \\
    &= O(n^{-3} \rho(\bh)^{-2}).
\end{align*}
...
\begin{align*}
    \tilde{A}_n &= O(n^{-1}\rho(\bh)^{-3} + \rho(\bh)^{-1})
\end{align*}
Also
\begin{align*}
    \tilde{B}_n &\defin \mathsf{E}[\tilde{H}_n^2(\bX_1,\bX_2)] \asymp \rho (\bh)^{-1}.
\end{align*}

Therefore,
\begin{align*}
    \frac{\tilde{A}_n}{\tilde{B}_n^2}=\frac{O(n^{-1}\rho(\bh)^{-3} + \rho(\bh)^{-1})}{O(\rho(\bh)^{-2})} = ((n\rho(\bh))^{-1}+\rho(\bh)) \to 0,
\end{align*}
which proves the theorem condition.
...
\begin{proof}[Proof of Lemma \ref{lemma-in2-e}]
...
\begin{align*}
        \mathsf{E} (T_{n,2}) &= \mathsf{E} \Bigg( \int_{\Sr} \Big[ \hat{f}_{0}(\bx;\bh) - \mathsf{E} (\hat{f}_{0}(\bx;\bh)) \Big]^2 \,\rd \bx \Bigg) \\
...
\begin{align*}
     \mathsf{E} (T_{n,2}) =&\; \int_{\Sr} \Bigg\{ \Bigg[ \sum_{j=1}^{r} \mathsf{Var} (\hat{f}_j(x^{(j)};h_j)) \Bigg( \prod_{\substack{k=1\\k\neq j}}^r \mathsf{E}^2 (\hat{f}_k(x^{(k)};h_k)) \Bigg) \Bigg] \\
...
Therefore,
\begin{align*}
    \mathsf{E} (T_{n,2}) &= \sum_{j=1}^{r} \int_{\Sr} \Bigg\{ \Bigg[ \frac{v_{d_j}(L)}{n h_j^{d_j}} f_j(x^{(j)})(1 + o(1)) \Bigg] \Bigg[ f_{0,-j}(\bx)^2+ O \Bigg( \sum_{j=1}^{r} h_j^2 \Bigg) \Bigg] \Bigg\} \,\rd \bx + o \Bigg( n^{-1} \sum_{j=1}^{r} h_j^{-d_j} \Bigg)\\
...
Therefore,
\begin{align*}
    \mathsf{E} (T_{n,2}) &= \sum_{j=1}^{r} \left\{\frac{v_{d_j}(L)}{n h_j^{d_j}}  R(f_{0,-j})+ o((n h_j^{d_j})^{-1})\right\}+ o \Bigg( n^{-1} \sum_{j=1}^{r} h_j^{-d_j} \Bigg)\\
...
\begin{proof}[Proof of Lemma \ref{lemma-in3-e}]
    Note that
    \begin{align*}
...
\end{align*}
Now notice that Equation \eqref{eq:sumrs} has $2^r -1$ terms once the two terms $\prod_{j=1}^{r} \mu_j(\bx^{(j)})$ cancel out. Without loss of generality, consider an arbitrary term $\theta_l$; note that this term can be written as
\begin{align*}
...
\begin{align*}
    ...
    =&\; \int_{\Sr} \sum_{l_1=1}^{2^r-1} \sum_{l_2=1}^{2^r-1} \theta_{l_1} \theta_{l_2} \,\rd \bx \\
    =&\; \sum_{l_1=1}^{2^r-1} \sum_{l_2=1}^{2^r-1} \int_{\Sr} \theta_{l_1} \theta_{l_2} \,\rd \bx \\
    =&\; \sum_{l_1=1}^{2^r-1} \sum_{l_2=1}^{2^r-1} \int_{\Sr} \lrb{\prod_{j=1}^{r} a_{l_1}^{(j)}} \lrb{\prod_{j=1}^{r} a_{l_2}^{(j)}} \,\rd \bx \\
    =&\; \sum_{l_1=1}^{2^r-1} \sum_{l_2=1}^{2^r-1} \int_{\Sr} \prod_{j=1}^{r} a_{l_1}^{(j)} a_{l_2}^{(j)} \,\rd \bx \\
    =&\; \sum_{l_1=1}^{2^r-1} \sum_{l_2=1}^{2^r-1} \prod_{j=1}^{r} \int_{\Sj} a_{l_1}^{(j)} a_{l_2}^{(j)} \,\rd \bx^{(j)}.
\end{align*}

Using Lemma \ref{lemma-sum-variances}, we have
\begin{align*}
    \mathsf{Var} (T_{n,2}) &= \sum_{l_1=1}^{2^r-1} \sum_{l_2=1}^{2^r-1} O \lrb{\mathsf{Var} \lrb{\prod_{j=1}^{r} \int_{\Sj} a_{l_1}^{(j)} a_{l_2}^{(j)} \,\rd \bx^{(j)}} }.
\end{align*}
...
The remaining terms have $a_{l_1}^{(j)} = \mu_j(\bx^{(j)})$ holding less than $r-1$ times or $a_{l_2}^{(j)} = \mu_j(\bx^{(j)})$ holding less than $r-1$ times; therefore, given that $\int_{\Sj} \Delta_{j}(\bx^{(j)}) \mu_{j}(\bx^{(j)}) \,\rd \bx^{(j)} \to 0$ and $\int_{\Sj} a_{l_1}^{(j)} a_{l_2}^{(j)} \,\rd \bx^{(j)} \to 0$, it follows that all the remaining therms will go faster to zero and, therefore, cannot be the dominant terms.
...
\begin{align*}
    \frac{n^{-2}h_{j}^{-d_{j}}}{n^{-2}} &= h_{j}^{-d_{j}} \to \infty,
\end{align*}
...
\begin{proof}[ Proof of Theorem \ref{teo-main}]
    ...
    Therefore,
     \begin{align*}
        T_{n,1} 
        &= \mathsf{E} (I_{n,2}) + I_{n,3} + o_{\mathsf{P}}(B_n^{1/2}) \\
        &= \mathsf{E}(T_{n,1}) + I_{n,3} + o_{\mathsf{P}}(B_n^{1/2}),
    \end{align*}
    where the last equality follows because $\mathsf{E}(I_{n,3})=0$.
    ...
    \begin{align*}
        \frac{n^{-2} \sum_{j=1}^{r} h_j^{-d_j}}{n^{-2} \rho (\bh)^{-1}}
    ...
    \begin{align*}
        T_n 
        &= \mathsf{E}(T_{n,1})+\mathsf{E}(T_{n,2})+\mathsf{E}(T_{n,3}) + I_{n,3} + o_{\mathsf{P}}(B_n^{1/2}) \\
        &= \mathsf{E}(T_n) + I_{n,3} + o_{\mathsf{P}}(B_n^{1/2}).
    \end{align*}
    Now remember that $I_{n,3} = B_n^{1/2}N_n$, where $N_n$ is asymptotically standard normal. Therefore, by Slutsky's theorem,
    \begin{align*}
        B_n^{-1/2}(T_n - \mathsf{E}(T_n)) \xrightarrow{d} \mathcal{N}(0, 1).
    \end{align*}

    Now we use the fact that $ B_n \asymp 2 n^{-2} \rho(\bh)^{-1} \lambda_{\bd}(L)^{-4}  \prod_{j=1}^{r} \sigma_j^2$ (Lemma \ref{lemma-sigma2}):
    \begin{align*}
        (n \rho (\bh)^{1/2})(T_n - \mathsf{E}(T_n)) \xrightarrow{d} \mathcal{N} \lrp{0, 2 \lambda_{\bd}(L)^{-4}  \prod_{j=1}^{r} \sigma_j^2},
    \end{align*}
    where $\sigma_j^2$ is defined in Lemma \ref{lemma-sigma2}.

    Finally,
    \begin{align*}
        \mathsf{E}(T_n)
        &= \frac{v_{\bd}(L)}{n \rho (\bh)}(1+o(1)) - A_n^{(1)}(1+o(1)).
    \end{align*}
\end{proof}
...
\begin{proof}[Proof of Lemma \ref{lemma-var-tn3}]
Defining
\[
