...
%-----------------------------------------------%
\author{Vinícius Litvinoff$^{1,3}$, Eduardo García-Portugués$^2$, and Bruno Ebner$^3$}
...
It is worth noting that, for the case with $r=2, d_1 = d_2 = 1, \kappa_1 = 0$ (two random vectors supported in the circle), the joint distribution of the angles belongs to the  Johnson--Wehrly family of distribution --- see \cite{Johnson1978,Wehrly1980}. This result is available in Proposition \ref{prop-js-family}.
    ...
    All simulations were performed for $\kappa_1 = 0$ ($\bmu$ uniform in the hypersphere), because, in this way, we have a single parameter $a$ measuring deviation from independence.
...
\end{align*}
    ...
    In the same way as in the previous model, we can define a parameter $a \defin \frac{\kappa}{\kappa + 1} \in [0,1)$ measuring deviation from independence, where $a=0$ represents independence.
\end{enumerate}

Scenario i is constructed based on the fact that conditional independence is a simple way to induce dependence between random vectors. Note that, as a particular case, when we have $a=0$, the variables $\bX^{(1)}, \dots, \bX^{(r)}$ become independent.

Scenario ii produces a sequence of random vectors, which of them supported in a hypersphere, where the location parameter of one component $\bX^{(j)}$ given the previous ones is given by the last component $\bX^{(j-1)}$. As in the first scenario, the parameter $a$ represents the amount of deviation from independence.

Each scenario is simulated $1000$ times using $B=100$ random permutations in each p-value computation. The adopted decision rule is to reject the null hypothesis when the p-value is less or equal to $\alpha = 0.05$. The bandwidths used in the simulation are proportional to the bandwidths chosen by likelihood cross-validation (see \cite{Hall1987}). So, if $h^*$ denotes the bandwidth selected by the aforementioned criterion, we see the empirical power of the test for bandwidths $2^c \times h^*$, where $c$ is a tuning parameter also studied in this simulation. A pilot simulation study showed that negative values of $c$ produced tests with poor control of type 1 error, and, for this reason, we used values $c \ge 0$.
...
\bottomrule
\end{tabular}
\caption{Empirical power ($\alpha=0.05$) for $r=2, d=1$ across sample sizes, tuning parameter $c$, and dependence parameter $a$.}
\end{table}

Figures \ref{fig:powercurve1}, \ref{fig:powercurve2} and \ref{fig:powercurve3} summarize some key interesting findings from the simulation study. First, it is possible to see the power increasing as function of the dependence parameter $a$, which is reasonable. Furthermore, it is possible to see that increasing the bandwidth has a negative effect on the power, under both null and alternative hypotheses. Finally, it is interesting to notice the effect of the dimension parameters $d$ and $r$: both affected negatively the test behavior, but the impact is much more moderate for the parameter $r$; increasing $d$ affected considerably the power.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Rplot20.pdf}
    \caption{Empirical power as a function of the significance level in the model ``Mixture of iid von Mises--Fisher distributions'', for $n=100$, $d=1$ and $r=2$; the parameter $a \in [0,1)$ represents deviation from independence, and $c = 0,1,2,3$ is a tuning parameter related to the degree of smoothness (see the text in this section). Axes in log-scale.} 
    \label{fig:powercurve1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Rplot21.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{Rplot22.pdf}
