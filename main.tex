...
\begin{proof}[ Proof of Theorem \ref{teo-main}]
    ...
    \begin{align*}
        B_n^{-1/2}(T_n - A(n)) \xrightarrow{d} \mathcal{N}(0, 1).
    \end{align*}

    Now we use the fact that $ B_n \asymp 2 n^{-2} \rho(\bh)^{-1} \lambda_{\bd}(L)^{-4}  \prod_{j=1}^{r} \sigma_j^2$ (Lemma \ref{lemma-sigma2}):
    \begin{align*}
        (n \rho (\bh)^{1/2})(T_n - A(n)) \xrightarrow{d} \mathcal{N} \lrp{0, 2 \lambda_{\bd}(L)^{-4}  \prod_{j=1}^{r} \sigma_j^2},
    \end{align*}
    where $A(n) \defin \frac{v_d(L)}{n \rho (\bh)} - A_n^{(1)}$, and $\sigma_j^2$ is defined in Lemma \ref{lemma-sigma2}.
